...
namespace SandBeige.MediaBox.Tests.ViewModels.Dialog {
	internal class DialogViewModelTest : ViewModelTestClassBase {
		...
		public void インスタンス生成() {
			using var vm = new DialogViewModel("title", "message", MessageBoxButton.YesNo);
		...
		public void ボタンパターン(MessageBoxButton button, params MessageBoxResult[] result) {
			using var vm = new DialogViewModel("title", "message", button);
			vm.ButtonList.Select(x => x.DialogResult).Is(result);
		}

		[Test]
		public void デフォルトボタンYes() {
			using var vm = new DialogViewModel("title", "message", MessageBoxButton.YesNoCancel, MessageBoxResult.Yes);
		...
		public void デフォルトボタンNo() {
			using var vm = new DialogViewModel("title", "message", MessageBoxButton.YesNoCancel, MessageBoxResult.No);
		...
		public void デフォルトボタンCancel() {
			using var vm = new DialogViewModel("title", "message", MessageBoxButton.YesNoCancel, MessageBoxResult.Cancel);
			vm.ButtonList.Single(x => x.IsDefault).DialogResult.Is(MessageBoxResult.Cancel);
		}

		[Test]
		public void デフォルトボタンok() {
			using var vm = new DialogViewModel("title", "message", MessageBoxButton.OKCancel, MessageBoxResult.OK);
