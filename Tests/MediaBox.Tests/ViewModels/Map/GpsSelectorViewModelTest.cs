...
namespace SandBeige.MediaBox.Tests.ViewModels.Map {
	...
	internal class GpsSelectorViewModelTest : ViewModelTestClassBase {
		...
		public async Task SetCandidateMediaFiles() {
			...
			var vm = Get.Instance<GpsSelectorViewModel>(model);
			var images = new[] {
				this.MediaFactory.Create(Path.Combine(TestDirectory, "image1.jpg")),
				this.MediaFactory.Create(Path.Combine(TestDirectory, "image2.jpg")),
				this.MediaFactory.Create(Path.Combine(TestDirectory, "image3.jpg"))
			};
