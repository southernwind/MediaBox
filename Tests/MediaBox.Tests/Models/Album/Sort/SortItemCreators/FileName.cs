...
namespace SandBeige.MediaBox.Tests.Models.Album.Sort.SortItemCreators {
	internal class FileName : ModelTestClassBase {
		...
		public void ソートテスト昇順() {
			...
			var si = sic.Create();
			si.ApplySort(this.CreateModels(), false).Select(x => x.MediaFileId).Is(5, 1, 2, 3, 4);
			si.ApplySort(this.CreateModels(), true).Select(x => x.MediaFileId).Is(4, 3, 2, 1, 5);
		}
		...
		public void ソートテスト降順() {
			...
			var si = sic.Create();
			si.ApplySort(this.CreateModels(), false).Select(x => x.MediaFileId).Is(4, 3, 2, 1, 5);
			si.ApplySort(this.CreateModels(), true).Select(x => x.MediaFileId).Is(5, 1, 2, 3, 4);
		}
