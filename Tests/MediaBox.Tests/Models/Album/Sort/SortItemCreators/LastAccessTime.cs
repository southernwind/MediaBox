...
namespace SandBeige.MediaBox.Tests.Models.Album.Sort.SortItemCreators {
	internal class LastAccessTime : ModelTestClassBase {
		...
		public void ソートテスト昇順() {
			var sic = new SortItemCreator(SortItemKeys.LastAccessTime);
			var si = sic.Create();
		...
		public void ソートテスト降順() {
			var sic = new SortItemCreator(SortItemKeys.LastAccessTime, ListSortDirection.Descending);
			var si = sic.Create();
