...
namespace SandBeige.MediaBox.Tests.Models.Album.Sort.SortItemCreators {
	internal class LastAccessTime : ModelTestClassBase {
		...
		public override void SetUp() {
			...
			mock5.Setup(x => x.LastAccessTime).Returns(new DateTime(2014, 6, 4, 0, 0, 0));
		...
		public void 追加ソートテスト昇順() {
			...
			var sic2 = new SortItemCreator(SortItemKeys.LastAccessTime);
			var si2 = sic2.Create();
		...
		public void 追加ソートテスト降順() {
			...
			var si = sic.Create();
			var ordered = si.ApplySort(this._testData, false);
			var sic2 = new SortItemCreator(SortItemKeys.LastAccessTime, ListSortDirection.Descending);
			var si2 = sic2.Create();
