...
namespace SandBeige.MediaBox.ViewModels.Album.Editor {
	...
	public class AlbumForEditorViewModel : ViewModelBase {
		...
		public AlbumForEditorViewModel(IAlbumForEditorModel model, IDialogService dialogService, ViewModelFactory viewModelFactory, IAlbumContainer albumContainer) {
			...
			this.CurrentItem = this._model.CurrentMediaFile.Select(x => x == null ? null : viewModelFactory.Create(x)).ToReadOnlyReactivePropertySlim().AddTo(this.CompositeDisposable);
