...
namespace SandBeige.MediaBox.Models.Media {
	public class VideoFileModel : MediaFileModel {
		...
		public VideoFileModel(string filePath, ISettings settings, ILogging logging, INotificationManager notificationManager, IVideoThumbnailService videoThumbnailService) : base(filePath, settings) {
		...
		public void CreateThumbnail(int index, double time) {
			if (this.Resolution is not { } r) {
				throw new InvalidOperationException();
			}
			this.IsAutoGeneratedThumbnail = false;
			try {
				this.RelativeThumbnailFilePath = this._videoThumbnailService.Create(this.FilePath, r, index, time);
		...
		public override void CreateThumbnail() {
			if (this.Duration is not { } d || this.Resolution is not { } r) {
				throw new InvalidOperationException();
			}
			try {
				this.RelativeThumbnailFilePath = this._videoThumbnailService.Create(this.FilePath, r, d);
				base.CreateThumbnail();
